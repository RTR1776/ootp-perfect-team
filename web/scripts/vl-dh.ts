/**
 * Best bats on the vs-LHP board — the platoon-DH question.
 *
 * shop-fit blends the boards 70/30 toward vs-RHP, which is right for an
 * everyday bat and wrong for a card bought to start only against lefties.
 * This ranks purely on the vs-LHP board, prints the vR side beside it so a
 * pure platoon card is visible as one, and marks what is already owned.
 *
...
(async () => {
  ...
  const ownedBats = pool.filter((c) => c.ownedFlag && c.vL != null).sort((a, b) => b.vL - a.vL);
  console.log(`\n=== vs-LHP board · ${YEAR} RE ${pr ? `· ${PARK_YEAR} ${PARK} (half)` : "· neutral"} · collection upload ${UP} ===`);
  console.log(`your own vs-LHP bar: #1 ${ownedBats[0].name} ${f1(ownedBats[0].vL)}, #5 ${f1(ownedBats[4].vL)}, #9 ${f1(ownedBats[8].vL)}, #14 ${f1(ownedBats[13].vL)}`);
  const hdr = `  ${"vsLHP".padStart(6)} ${"vsRHP".padStart(6)} ${"split".padStart(6)} ${"val".padStart(4)} ${"B"} ${"year".padStart(5)} ${"name".padEnd(24)} pos    price`;
  const line = (c: any) => { const p = price(c);
    console.log(`  ${f1(c.vL).padStart(6)} ${f1(c.vR).padStart(6)} ${f1(c.vL - c.vR).padStart(6)} ${String(c.val).padStart(4)} ${c.bats} ${String(c.year ?? "").padStart(5)} ${String(c.name).slice(0,24).padEnd(24)} ${String(c.role ?? "").padEnd(4)} ${c.ownedFlag ? "  OWNED" : p == null ? "     —" : String(p).padStart(8)}${!c.ownedFlag && p != null && p <= BUDGET ? "  YES" : ""}`); };

  if (NAME) {
    const hits = pool.filter((c) => String(c.name).toLowerCase().includes(NAME.toLowerCase()));
    const rankL = pool.filter((c) => c.vL != null).sort((a, b) => b.vL - a.vL);
    console.log(`\n--- "${NAME}" ---`); console.log(hdr);
    for (const h of hits.sort((a,b)=>b.vL-a.vL)) { line(h); console.log(`        rank ${rankL.indexOf(h)+1} of ${rankL.length} on the vs-LHP board`); }
  }
  console.log(`\n--- your 10 best owned bats vs LHP ---`); console.log(hdr); ownedBats.slice(0,10).forEach(line);
  const buy = pool.filter((c) => !c.ownedFlag && c.vL != null && (price(c) ?? 1e9) <= BUDGET).sort((a, b) => b.vL - a.vL);
  console.log(`\n--- best UNOWNED bats vs LHP at or under ${BUDGET.toLocaleString()} PP ---`); console.log(hdr); buy.slice(0, SHOW).forEach(line);
  process.exit(0);
})();
