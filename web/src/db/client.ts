 ...
 */
const globalForDb = globalThis as unknown as { __ootpPool?: Pool };

/**
 * Typed as the node-postgres flavour rather than a union of the two.
 *
 * The two drivers expose the same query-builder API but not identical generic
 * signatures, and a union of them collapses overloads like `.returning(...)`
 * down to their incompatible intersection — which fails to typecheck at every
 * call site even though both drivers accept the call at runtime. One concrete
 * type keeps callers honest; the runtime choice is still made below.
 */
export const db: NodePgDatabase<typeof schema> = isNeon(connectionString)
  ? (drizzleNeon(neon(connectionString), { schema }) as unknown as NodePgDatabase<typeof schema>)
  : drizzlePg(
