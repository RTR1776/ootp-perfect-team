...
interface Row { u: UpgradeCard; gain: number; where: string; price: number | null; perK: number | null; overCap: number }
      ...
      if (!u.isPitcher) {
        for (const hand of ["R", "L"] as const) {
          ...
          for (const pos of p.lineupPos) {
            ...
            if (d > best) { best = d; at = `${pos} vs ${hand}HP over ${cur.name}`; rep = cur; }
          }
        ...
        if (gSp >= gRp && gSp > 0 && sp) { gain = gSp; where = `rotation over ${sp.c.name}`; replaced = sp.c; }
        else if (gRp > 0 && rp) { gain = gRp; where = `bullpen over ${rp.c.name}`; replaced = rp.c; }
      }
      if (gain <= 0.05) continue;
      const price = u.ask && u.ask > 0 ? u.ask : u.last10;
      const overCap = p.teamCap != null ? Math.max(0, capUsed - (replaced?.val ?? 0) + (u.val ?? 0) - p.teamCap) : 0;
      out.push({ u, gain, where, price: price ?? null, perK: price ? gain / (price / 10000) : null, overCap });
    }
    return out.sort((a, b) => b.gain - a.gain);
  }, [p.upgrades, p.slots, p.lineupPos, p.spKeys, p.rpKeys, p.byId, p.runsR, p.runsL, p.lhpShare, p.teamCap, onBoard.length, capUsed]);
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="font-mono text-[12.5px] leading-tight [font-variant-numeric:tabular-nums]">
            {shown.slice(0, 60).map((r) => (
              <tr key={r.u.cardId} className="border-b border-border/50">
