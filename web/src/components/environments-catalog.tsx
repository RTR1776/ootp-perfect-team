 ...
 *
 * These rows are the SCHEDULED snapshot from src/data/environments.json and are
 * neutral-park. Tournaments do run in real ballparks and both the era and the
 * park rotate through the year — the live era+park read, solved fresh, is the
 * Similar-environments card above (src/components/env-similarity.tsx).
 */
...
export function EnvironmentsCatalog() {
    ...
    <div className="flex flex-col gap-4">
      <h2 className="text-base font-semibold tracking-tight">
        ...
        <span className="ml-2 text-xs font-normal text-muted-foreground">
          {environments.length} run environments · {environments.reduce((s, e) => s + e.events.length, 0)} tournaments ·
          rules + the preset to set before first pitch
        </span>
      </h2>
      ...
      <p className="text-xs text-muted-foreground">
        ...
        environment only. These rows are neutral-park and reflect the schedule as of the snapshot — for the current
        era + ballpark of a specific event, use the search above.
      </p>
    </div>
  );
}
