...
| New cards out (art missing on /cards or the /build peek) | **Upload Card Art.command**. The first run asks for the Blob token and saves it to `web/.env.blob`. After that, File OOTP Exports.command runs it when it quits. | `node scripts/upload-card-art.mjs` reads OOTP's card cache (`~/Application Support/…/online_data/cache/cards`), checks each card against the store, uploads what is missing, and lists app cards the cache does not have yet (open those once in the game) | Blob store `cards/<cid>.webp`; `web/scripts/.card-art-uploaded.json` (gitignored) |
